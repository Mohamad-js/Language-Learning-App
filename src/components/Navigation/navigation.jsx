...
function Navigation(){
   ...
   return(
      <div className='fixed bottom-0 left-0 w-full grid grid-cols-6 backdrop-blur-xs h-15 border-t border-foreground bg-background z-1 px-5'>
         {tabs.map((level, index) => (
            ...
            >
...

               {
                  active === index && (
                     <motion.div 
                        className="absolute -top-5 size-12 rounded-2xl bg-background border border-foreground"
                        layoutId="bubble"
                        transition={{
                           type: 'spring',
                           stiffness: 900,
