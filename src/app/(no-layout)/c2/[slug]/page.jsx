...
import { c2WordList } from '@/data/c2WordList';
...
export default function Lessons({ params }) {
   ...
   const [savedC2Vocabs, setSavedC2Vocabs] = useState([])
   ...
   const [c2WordsCount, setC2WordsCount] = useState(null)
   const [lessonsC2, setLessonsC2] = useState(null)
...
// Retrieve data from localStorage on mount
   useEffect(() => {
      try {
         setWordList(c2WordList)

         const savedKnowns = JSON.parse(localStorage.getItem(`knownWords-${slug}-C2`) || '[]');
         const savedUnknowns = JSON.parse(localStorage.getItem(`unknownWords-${slug}-C2`) || '[]');
         ...
         const totalWordsLearnt = (slug * 10) + Number(a1WordsLearnt) + Number(a2WordsLearnt) + Number(b1WordsLearnt) + Number(b2WordsLearnt) + Number(c1WordsLearnt)
         const c2WordsLearnt = slug * 10
         
         setC2WordsCount(c2WordsLearnt)
         setPreview(previewState)
         setTotalWordsCount(totalWordsLearnt)
         setLessonsC2(lessonsProgress)
   ...
   const closeCongrats = () => {
      ...
      btnPressed === 'done' ? router.push('/c2') :
      btnPressed === 'nextLesson' ? router.push(`/c2/${lessonNumber + 1}`) :
      btnPressed === 'nextLevel' ? router.push('/c2') : console.log('PROBLEM')
   }

   const save = () => {
      localStorage.setItem(`knownWords-${slug}-C2`, JSON.stringify(knownWords));
      localStorage.setItem(`unknownWords-${slug}-C2`, JSON.stringify(unknownWords));
      localStorage.setItem(`wordsCount-C2`, JSON.stringify(c2WordsCount));
      localStorage.setItem(`totalWordsCount`, JSON.stringify(totalWordsCount));
      localStorage.setItem(`currentLesson-C2`, JSON.stringify(slug));
   }
   ...
   useEffect(() => {
      try {
         localStorage.setItem(`savedC2Vocabs-${slug}-C2`, JSON.stringify(savedC2Vocabs));
   ...
   const saveHandle = (ws) => {
      ...
      if(savedC2Vocabs.some(item => item.word == ws.word.word) && savedC2Vocabs.some(item => item.role == ws.word.role)){
         setSavedC2Vocabs(savedC2Vocabs.filter((item) => {
   ...
   const handleAnswer = (status) => {
      ...
      if (status === 'known') {
         setKnownWords([...knownWords, { word: currentWord, type: status, lesson: lessonNumber, level: 'C2' }]);
      } else if (status === 'unknown') {
         setUnknownWords([...unknownWords, { word: currentWord, type: status, lesson: lessonNumber, level: 'C2' }]);
      }
   ...
   return (
      <div className={styles.container}>
         ...
         <div className={styles.lessonLevel}>C2</div>
               ...
               <div className={`${styles.done} ${show && styles.show}`}>
                  ...
                  <div className={styles.btnHolder}>
                     <button className={styles.back} onClick={done}>Save</button>
                     {
                        ...
                        :
                        <button className={styles.back} onClick={nextLevel}>Start C2</button>
                     }
                  </div>
               </div>
            )
               ...
               <>
                  <div className={styles.actionsHolder}>
                     ...
                     <p className={styles.actions} onClick={() => saveHandle(ws)}>
                        {  
                           (savedC2Vocabs.some(item => item.word == ws.word.word) && savedC2Vocabs.some(item => item.role == ws.word.role)) ? <FaBookmark className={styles.save}/> : <FaRegBookmark className={styles.save}/>  
                        }
                     </p>
                  </div>
                        ...
                        btn ? 
                           <div className={styles.btnHolder}>
                              ...
                              lessonNumber < wholeLessons ?
                                 ...
                                 :
                                 <button className={styles.button} onClick={nextLevel}>Start C2</button>  
                              }
